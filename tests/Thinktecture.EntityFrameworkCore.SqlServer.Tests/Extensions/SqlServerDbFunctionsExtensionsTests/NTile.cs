...
{
   public NTile(ITestOutputHelper testOutputHelper, SqlServerFixture sqlServerFixture)
   ...
   public void Generates_NTile_with_orderby_and_two_buckets()
   {
      ...
      result.First(t => t.Name == "1").Bucket.Should().Be(1L);
      result.First(t => t.Name == "2").Bucket.Should().Be(2L);
   }
   ...
   public void Generates_NTile_with_orderby_desc()
   {
      ...
      result.First(t => t.Name == "1").Bucket.Should().Be(2L);
      result.First(t => t.Name == "2").Bucket.Should().Be(1L);
   }
   ...
   public void Generates_NTile_with_partitionby_and_orderby()
   {
      ...
      result.First(t => t.Name == "A" && t.Count == 1).Bucket.Should().Be(1L);
      result.First(t => t.Name == "A" && t.Count == 2).Bucket.Should().Be(2L);
      result.First(t => t.Name == "B" && t.Count == 1).Bucket.Should().Be(1L);
      result.First(t => t.Name == "B" && t.Count == 2).Bucket.Should().Be(2L);
   }
