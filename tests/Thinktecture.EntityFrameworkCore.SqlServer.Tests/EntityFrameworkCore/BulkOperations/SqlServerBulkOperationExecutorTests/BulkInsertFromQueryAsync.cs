...
public class BulkInsertFromQueryAsync : IntegrationTestsBase
{
   ...
   public BulkInsertFromQueryAsync(ITestOutputHelper testOutputHelper, SqlServerFixture sqlServerFixture)
   ...
   public async Task Should_insert_entities_from_temp_table()
   {
      await ActDbContext.Database.ExecuteSqlRawAsync($"""
         CREATE TABLE [{Schema}].[TestEntities_QueryInsertRedirect] (
      ...
      try
      {
         ...
         var affectedRows = await ActDbContext.Set<TestEntity>().BulkInsertAsync(
            ...
            new SqlServerBulkInsertFromQueryOptions { TableName = "TestEntities_QueryInsertRedirect", Schema = Schema });
   ...
   public async Task Should_insert_private_field_via_ef_property()
   {
      await ActDbContext.Database.ExecuteSqlRawAsync($"""
         CREATE TABLE [{Schema}].[TestEntities_QueryInsertRedirect] (
            ...
            [Name] nvarchar(max) NULL,
            [Count] int NOT NULL,
            [PropertyWithBackingField] int NOT NULL DEFAULT 0,
            [_privateField] int NOT NULL DEFAULT 0,
            CONSTRAINT [PK_TestEntities_QueryInsertRedirect] PRIMARY KEY ([Id])
         );
         """);
