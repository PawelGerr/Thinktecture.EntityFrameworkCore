...
public class BulkUpdateAsync : IntegrationTestsBase
{
   ...
   public async Task Should_update_entities_in_table_name_override()
   {
      await ActDbContext.Database.ExecuteSqlRawAsync($"""
         SELECT TOP 0 * INTO [{Schema}].[TestEntities_BulkUpdateRedirect] FROM [{Schema}].[TestEntities];
         ALTER TABLE [{Schema}].[TestEntities_BulkUpdateRedirect] ADD PRIMARY KEY ([Id]);
      ...
      try
      {
         ...
         var affectedRows = await SUT.BulkUpdateAsync(new[] { entity }, new SqlServerBulkUpdateOptions { TableName = "TestEntities_BulkUpdateRedirect" });
   ...
   public async Task Should_throw_when_trying_to_update_temp_table_entity_without_table_name()
   {
      ...
      await SUT.Invoking(sut => sut.BulkUpdateAsync(new List<TempTable<int>> { new(0) }, new SqlServerBulkUpdateOptions()))
   ...
   public async Task Should_update_temp_table_entity_when_table_name_is_provided()
   {
      ...
      var affectedRows = await SUT.BulkUpdateAsync(new[] { tempEntity }, new SqlServerBulkUpdateOptions { TableName = "TestEntities" });
