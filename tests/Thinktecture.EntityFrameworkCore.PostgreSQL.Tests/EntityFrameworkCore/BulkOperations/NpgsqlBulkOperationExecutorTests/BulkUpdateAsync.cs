...
public class BulkUpdateAsync : IntegrationTestsBase
{
   ...
   public BulkUpdateAsync(ITestOutputHelper testOutputHelper, NpgsqlFixture npgsqlFixture)
   ...
   {
      var affectedRows = await SUT.BulkUpdateAsync(new List<TestEntity>(), new NpgsqlBulkUpdateOptions());
   ...
   public async Task Should_update_entities()
   {
      ...
      var affectedRows = await SUT.BulkUpdateAsync([entity_1, entity_2], new NpgsqlBulkUpdateOptions());
   ...
   public async Task Should_return_0_if_no_rows_match()
   {
      ...
      var affectedRows = await SUT.BulkUpdateAsync(new List<TestEntity> { new() { Id = new Guid("506E664A-9ADC-4221-9577-71DCFD73DE64"), RequiredName = "RequiredName" } }, new NpgsqlBulkUpdateOptions());
   ...
   public async Task Should_update_provided_entity_only()
   {
      ...
      var affectedRows = await SUT.BulkUpdateAsync([entity_1], new NpgsqlBulkUpdateOptions());
   ...
   public async Task Should_update_specified_properties_only()
   {
      ...
      var affectedRows = await SUT.BulkUpdateAsync([entity],
                                                   new NpgsqlBulkUpdateOptions
                                                   {
   ...
   public async Task Should_update_entities_based_on_non_pk_property()
   {
      ...
      var affectedRows = await SUT.BulkUpdateAsync([entity_1, entity_2], new NpgsqlBulkUpdateOptions { PropertiesToUpdate = properties, KeyProperties = keyProperties });
   ...
   public async Task Should_update_private_property()
   {
      ...
      var affectedRows = await SUT.BulkUpdateAsync([entity], new NpgsqlBulkUpdateOptions());
   ...
   public async Task Should_update_column_with_converter()
   {
      ...
      var affectedRows = await SUT.BulkUpdateAsync(new List<TestEntity> { entity }, new NpgsqlBulkUpdateOptions());
   ...
   [Fact]
   public async Task Should_update_entity_with_ComplexType()
   {
      ...
      await SUT.BulkUpdateAsync([testEntity], new NpgsqlBulkUpdateOptions());
   ...
   public async Task Should_update_entities_in_table_name_override()
   {
      await ActDbContext.Database.ExecuteSqlRawAsync($"""
         CREATE TABLE "{Schema}"."TestEntities_BulkUpdateRedirect" (LIKE "{Schema}"."TestEntities" INCLUDING ALL);
         INSERT INTO "{Schema}"."TestEntities_BulkUpdateRedirect" ("Id", "RequiredName", "Name", "Count", "NullableCount", "ConvertibleClass", "ParentId", "PropertyWithBackingField", "_privateField")
      ...
      try
      {
         ...
         var affectedRows = await SUT.BulkUpdateAsync([entity], new NpgsqlBulkUpdateOptions { TableName = "TestEntities_BulkUpdateRedirect", Schema = Schema });

         affectedRows.Should().Be(1);

         var redirectedNames = await AssertDbContext.Database
                                                   .SqlQueryRaw<string>($"""SELECT "Name" FROM "{Schema}"."TestEntities_BulkUpdateRedirect" """)
                                                   .ToListAsync();
      ...
      {
         await ActDbContext.Database.ExecuteSqlRawAsync($"""DROP TABLE IF EXISTS "{Schema}"."TestEntities_BulkUpdateRedirect" """);
      }
   }

   [Fact]
   public async Task Should_update_entity_with_jsonb_and_json_columns()
   {
      ...
      entity.JsonbColumn = """{"key": "updated"}""";
      entity.JsonColumn = """{"key": "updated"}""";
