...
public class MaxMinUuidWindowFunction : IntegrationTestsBase
{
   ...
   public MaxMinUuidWindowFunction(ITestOutputHelper testOutputHelper, NpgsqlFixture npgsqlFixture)
   ...
   public async Task Should_translate_windowed_Max_over_uuid()
   {
      ...
      ExecutedCommands.Last().Should().Contain("MAX").And.Contain("text").And.Contain("uuid").And.Contain("OVER (");
   }
   ...
   public async Task Should_translate_windowed_Min_over_uuid()
   {
      ...
      ExecutedCommands.Last().Should().Contain("MIN").And.Contain("text").And.Contain("uuid").And.Contain("OVER (");
   }
